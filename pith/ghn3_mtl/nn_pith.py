...
class GHN3(GHN):
    ...
    def __init__(self, max_shape, task_embedding_dim, num_classes=10, hid=128, heads=8, layers=3, is_ghn2=False, pretrained=False, 
        ...
        # Learnable fusion weight, initialized to favor GHN output.
        # sigmoid(-1.5) ≈ 0.18, meaning initially 82% GHN + 18% teacher.
        self.fusion_weight = nn.Parameter(torch.tensor(-1.5))

        # Cache for preprocessed teacher parameters (excluded from state_dict).
        self._processed_teacher_params = {}
        ...
        if lora:
            ...
            self.decoder = ConvDecoder3LoRA_Gradual(in_features=hid,
                                        hid=(hid * 4, hid * 8),
                                        ck=max_ck_lora,
                                        r=lora_r,
                                        num_classes=num_classes)
            
            self.decoderpq = ConvDecoder3LoRA(in_features=hid,
                                        hid=(hid * 4, hid * 8),
                                        ck=max_ck_lora,
                                        r=lora_r,
                                        num_classes=num_classes)
    ...
    def forward(self,
        ...
        ## Handle teacher model (if provided in forward pass).
        if teacher_model is not None:
            # Only preprocess on first encounter.
            if not self._processed_teacher_params:
                # Load and preprocess teacher parameters.
                teacher_state_dict = self._load_teacher_model(teacher_model)
                if teacher_state_dict is not None:
                    # Store temporarily during preprocessing only.
                    self.teacher_state_dict = teacher_state_dict
                    self._processed_teacher_params = self._preprocess_teacher_params()
                    # Delete immediately to avoid being saved in state_dict.
                    del self.teacher_state_dict

            # Use cached preprocessed parameters.
            self.processed_teacher_params = self._processed_teacher_params
        else:
            # No teacher: use empty dict.
            self.processed_teacher_params = {}
        ...
        if embeddings is None:
            ...
            x_gnn = self.gnn(x, task_embedding, graphs.edges, mask)
            x = x_gnn[0]  # gnn returns (x, task_embedding) tuple
        ...
        for key, inds in param_groups.items():
            ...
            # Retrieve teacher parameter tensor for this group.
            teacher_params_batch = self._get_teacher_params_batch(
                param_groups, params_map, key, graphs
            )
            ...
            if len(sz) in [2, 3]:
                ...
                mt, nt = teacher_params_batch.shape[1], teacher_params_batch.shape[2]
                ms, ns = sz[0], sz[1]
                c1, c2 = sz[2], sz[3]  # spatial dimensions

                # 4D shape passed; only the first two dimensions are actually used.
                p_shape = (ms, mt, 1, 1)  # last two dims set to 1
                q_shape = (nt, ns, 1, 1)  # last two dims set to 1

                with autocast(enabled=False):
                    w = self.decoder(x_, sz, class_pred=False)  # base GHN prediction
                    p = self.decoderpq(x_, p_shape, class_pred=False)
                    q = self.decoderpq(x_, q_shape, class_pred=False)

                p = p.squeeze(-1).squeeze(-1)
                q = q.squeeze(-1).squeeze(-1)
                w_teacher = torch.einsum('bij,bjklm,bkn->binlm', p, teacher_params_batch, q)

                # Adaptive fusion: weighted combination of GHN and teacher predictions.
                alpha = torch.sigmoid(self.fusion_weight)
                w = (1 - alpha) * w + alpha * w_teacher
    ...
    def _load_teacher_model(self, teacher_model):
        """
        Load and manage the teacher model.
        :param teacher_model: path to the teacher model or an already-loaded model.
        :return: state_dict of the teacher model.
        """
        if teacher_model is None:
            return None
            
        if isinstance(teacher_model, str):
            teacher_state_dict = torch.load(teacher_model, map_location='cpu')
            return teacher_state_dict
        elif hasattr(teacher_model, 'state_dict'):
            return teacher_model.state_dict()
        else:
            return teacher_model

    def _preprocess_teacher_params(self):
        """
        Preprocess all teacher parameters by applying the _map_net_params transformation logic.
        Returns: dict of {param_name: transformed_tensor}.
        """
        processed_params = {}

        for param_name, teacher_param in self.teacher_state_dict.items():
            # Skip classification head.
            if "head" in param_name:
                continue

            # Infer parameter type.
            last_weight, last_bias = self._infer_param_type(param_name, teacher_param.shape)

            # Apply transformation logic.
            transformed_param = self._apply_map_net_params_transform(
                teacher_param, last_weight, last_bias
            )

            processed_params[param_name] = transformed_param

            if self.debug_level > 0:
                print(f"Teacher preprocess: {param_name}")
                print(f"  original shape: {teacher_param.shape}")
                print(f"  transformed shape: {transformed_param.shape}")
                print(f"  last_weight: {last_weight}, last_bias: {last_bias}")

        return processed_params

    def _apply_map_net_params_transform(self, param, last_weight, last_bias):
        """
        Apply the _map_net_params transformation logic to a single parameter.
        """
        import numpy as np

        sz = param.shape

        # Follow _map_net_params transformation logic.
        if len(sz) == 1:
            # 1D parameters: no transformation needed.
            return param

        elif last_weight:
            # last_weight stays 2D, dimensions unchanged.
            return param

        elif len(sz) == 2:
            # 2D -> 4D (required dimension expansion).
            return param.unsqueeze(2).unsqueeze(3)

        elif len(sz) == 3:
            if sz[0] == 1 and min(sz[1:]) > 1:  # positional encoding
                s = int(np.floor(sz[1] ** 0.5))
                # Preserve original hidden dimension.
                return self._transform_positional_encoding(param, sz[2], s)
            else:
                # Regular 3D: keep original dimensions.
                return param
        else:  # 4D
            # 4D conv: keep original dimensions.
            return param

    def _infer_param_type(self, param_name, shape):
        """
        Infer the parameter type from its name and shape.
        """
        is_bias = 'bias' in param_name.lower()
        is_last_layer = any(keyword in param_name.lower() 
                        for keyword in ['head', 'classifier', 'fc', 'linear'])
        
        last_weight = is_last_layer and not is_bias and len(shape) == 2
        last_bias = is_bias
    ...
    def _resize_tensor(self, tensor, target_shape):
        """
        Resize a tensor to the given target shape.
        """
        current_shape = tensor.shape
        result = tensor

        for dim, (current_size, target_size) in enumerate(zip(current_shape, target_shape)):
            if current_size > target_size:
                ...
                # expand by repeating
                repeat_times = [1] * result.ndim
                repeat_times[dim] = (target_size + current_size - 1) // current_size
                result = result.repeat(repeat_times)
    ...
    def _resize_conv4d_tensor(self, tensor, new_dims):
        """
        Resize the first two dimensions of a 4D convolution tensor.
        """
        new_dim0, new_dim1 = new_dims
        old_shape = tensor.shape

        # Reshape to 2D for processing.
        reshaped_2d = tensor.reshape(old_shape[0], old_shape[1], -1)
        target_2d_shape = (new_dim0, new_dim1, reshaped_2d.shape[2])
        resized_2d = self._resize_tensor(reshaped_2d, target_2d_shape)

        # Restore to 4D.
        return resized_2d.reshape(new_dim0, new_dim1, old_shape[2], old_shape[3])

    def _transform_positional_encoding(self, param, new_hidden, s):
        """
        Handle special transformation for positional encodings.
        """
        batch_size, seq_len, hidden_dim = param.shape

        # Resize hidden dimension.
        if hidden_dim != new_hidden:
            resized_param = self._resize_tensor(param, (batch_size, seq_len, new_hidden))
        else:
            resized_param = param
        ...
        if seq_len != target_seq_len:
            resized_param = self._resize_tensor(resized_param, (batch_size, target_seq_len, new_hidden))

        # Rearrange: [1, H*W, hidden] -> [1, hidden, H, W].
        return resized_param.permute(0, 2, 1).reshape(batch_size, new_hidden, s, s)

    def _get_teacher_param(self, param_name):
        """
        Retrieve the preprocessed teacher parameter directly by name.
        """
        if not hasattr(self, 'processed_teacher_params'):
            return None

        # Reverse-map parameter name.
        teacher_param_name = self._reverse_map_param_name(param_name)

        # Retrieve from preprocessed parameters.
        return self.processed_teacher_params.get(teacher_param_name, None)

    def _reverse_map_param_name(self, student_param_name):
        """
        Reverse mapping: convert a student parameter name back to the teacher parameter name,
        undoing the transformations applied by _map_net_params.
        """
        teacher_name = student_param_name

        # 1. Reverse attention module renaming.
        teacher_name = teacher_name.replace('attn.to_qkv', 'to_qkv')
        teacher_name = teacher_name.replace('attn.to_out', 'to_out')

        # 2. Reverse auto-appended .weight suffix.
        import re

        auto_added_weight_patterns = [
            r'\.pos_embedding\.weight$',
            r'\.class_token\.weight$',
            r'\.pos_embed\.weight$',
            r'\.cls_token\.weight$',
        ]

        for pattern in auto_added_weight_patterns:
            if re.search(pattern, teacher_name):
                teacher_name = teacher_name[:-7]  # remove '.weight'
                break

        return teacher_name

    def _extract_layer_index(self, param_name):
        """
        Extract the layer index from a parameter name.
        """
        import re
        match = re.search(r'encoder_layer_(\d+)', param_name)
        if match:
            return int(match.group(1))
        return None

    def _map_student_layer_to_teacher(self, student_layer_idx, student_total_layers, teacher_total_layers):
        """
        Map a student layer index to the corresponding teacher layer index.
        """
        if student_layer_idx < teacher_total_layers:
            return student_layer_idx
        else:
            return teacher_total_layers - 1
    def _get_teacher_params_batch(self, param_groups, params_map, key, graphs=None):
        """
        Retrieve preprocessed teacher parameters for the current key, returned as a tensor.
        """
        inds = param_groups[key]
        teacher_params = []

        for ind in inds:
            matched_info, shape_key, w_ind = params_map[ind]

            if w_ind is None:
                # No matching teacher parameter for this index.
                teacher_params.append(None)
                continue

            # Retrieve directly from preprocessed parameters.
            student_param_name = matched_info.get('param_name', 'unknown')
            teacher_param = self._get_teacher_param(student_param_name)
            teacher_params.append(teacher_param)

            if self.debug_level > 0:
                if teacher_param is not None:
                    print(f"Found preprocessed teacher: {student_param_name} -> {teacher_param.shape}")
                else:
                    print(f"Teacher parameter not found: {student_param_name}")

        # Filter out None values and convert to tensor.
        valid_teacher_params = [p for p in teacher_params if p is not None]

        if len(valid_teacher_params) == 0:
            return None

        # Convert to tensor (assumes all parameters share the same shape).
        teacher_params_tensor = torch.stack(valid_teacher_params)

        return teacher_params_tensor
    def _reshape_teacher_params_batch(self, teacher_params_batch, last_weights, last_biases, param_groups, params_map, key):
        """
        Teacher parameters are already preprocessed; assemble the batch directly.
        """
        if teacher_params_batch is None:
            return None, None

        # Filter and assemble.
        valid_params = [p for p in teacher_params_batch if p is not None]
        if len(valid_params) == 0:
            return None, None

        return torch.stack(valid_params), None
...
class ConvDecoder3LoRA_Gradual(ConvDecoder3LoRA):
    """
    Modified ConvDecoder3LoRA with gradual expansion: r -> r*2 -> r*4 -> ck
    Instead of directly jumping from r to ck, this version gradually increases the dimension.
    """
    def __init__(self, in_features, ck=32, r=32, hid=(64,), num_classes=None):
        # Initialize parent class, then replace l2 and seq.
        ...
        # Remove parent's l2 and seq; replace with gradual expansion layers.
        delattr(self, 'l2')
        delattr(self, 'seq')

        # Gradual expansion layers: r -> r*2 -> r*4 -> ck.
        self.l2_step1 = nn.Linear(r, r * 2)      # r -> r*2
        self.l2_step2 = nn.Linear(r * 2, r * 4)  # r*2 -> r*4
        self.l2_step3 = nn.Linear(r * 4, ck)     # r*4 -> ck

        # ReLU activation.
        self.relu = nn.ReLU(inplace=True)

        # Build gradual expansion sequence.
        self.seq = nn.Sequential(
            self.relu,
            self.l2_step1,
            self.relu,
    ...
    def forward(self, x, max_shape=(1,1,1,1), class_pred=False, n_dim=4, nets_torch=None):
        if class_pred:
            n_dim = 2

        # Process through parent MLP.
        x = self.mlp(x).view(-1, 2*self.r, self.r)  # [b, 2*r, r]

        # Apply gradual expansion: r -> r*2 -> r*4 -> ck.
        x = self.seq(x).view(-1, 2*self.ck, self.r)  # [b, 2*ck, r]

        # Remaining steps same as parent.
        A, B_t = torch.split(x, self.ck, dim=1)  # A=[b, ck, r] and B=[b, ck, r]
        B = B_t.transpose(1, 2)  # A=[b, ck, r] and B=[b, r, ck]

        # Crop A and B according to max_shape.
        c_out, c_in, k_out, k_in = max_shape
        A = A[:, :(c_out*k_out), :]  # [b, c_out*k_out, r]
        B = B[:, :, :(c_in*k_in)]    # [b, r, c_in*k_in]
        ...
        # Reshape output according to dimension requirement.
        if n_dim == 1:  # 1D parameter
            assert c_in == 1 and k_out == 1 and k_in == 1
            W = W.reshape(-1, c_out)
        elif n_dim == 2:  # 2D parameter
            pass
        elif n_dim == 4:  # 4D conv parameter
            W = W.reshape(-1, c_out, k_out, c_in, k_in).transpose(2, 3)  # [b, c_out, c_in, k_out, k_in]
        else:
            raise NotImplementedError("n_dim must be 1, 2, or 4")
