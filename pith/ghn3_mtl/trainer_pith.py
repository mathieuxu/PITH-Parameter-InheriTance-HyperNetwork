...
from .nn_sa_add_grad_large import from_pretrained
from .ops import Network
# from torchvision.models.vision_transformer import _vision_transformer
from vit import _vision_transformer
try:
...
class Trainer:
    ...
    # def update(self, images, targets, models = None, graphs=None, embeddings=None, param_groups_map=None, ghn=None, teacher_model=None):
    def update(self, images, targets, num_class, task_embedding, dataset_name, 
            models=None, graphs=None, embeddings=None, param_groups_map=None, 
            ghn=None, teacher_model=None, is_imagenet=False):
                ...
                if self._is_ghn:
                    ...
                    if hasattr(graphs, 'nets') and len(graphs.nets) > 0:
                        ...
                        for nets_args in graphs.net_args:
                            ...
                            net = _vision_transformer(
                                patch_size = 16, ## cifar10: patchs-size=2; imagenet: 16
                                num_layers = nets_args['num_layers'],
                                num_heads = nets_args['num_heads'],
                                hidden_dim = nets_args['hidden_dim'],
                                mlp_dim = nets_args['mlp_dim'],
                                # num_classes = 1000, # cifar: 100, imagenet: 1000
                                num_classes = num_class,  # dynamic class count (multi-task)
                                image_size = 224,  # cifar: 32, imagenet: 224
                                weights = None,
                                progress = False,
                                ).to(self.main_device)
                    ...
                    task_embedding = task_embedding.to(self.main_device)
                    ...
                    models = self._model(models,
                                         ...
                                         keep_grads=True,
                                         reduce_graph=True,
                                         teacher_model=teacher_model or self.teacher_model
                                         )
                    if self.predparam_wd > 0:
                        total_norm = 0
                        for m in models:
    ...
    def save(self, epoch, step, config, save_freq=2000, interm_epoch=5):
    ...
    def log(self, step=None):
        ...
        if step_ % self.log_interval == 0 or step_ >= self.n_batches - 1 or step_ == 1:
            ...
            metrics['dataset_name'] = self.dataset_name
