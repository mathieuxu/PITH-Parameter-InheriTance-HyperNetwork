...
from .imagenet_test import ImageNetDataset
...
def image_loader(dataset='imagenet', data_dir='./data/', test=True, im_size=32,
    ...
    if dataset.lower() == 'imagenet':
        ...
        #valid_data = ImageNetDataset(imagenet_dir, 'test', transform=valid_transform, has_validation=not test)
    ...
    if train_data is None:
        ...
        train_loader = DataLoader(
            ...
            prefetch_factor=prefetch_factor,  # 🔥 添加
            persistent_workers=persistent_workers if num_workers > 0 else False,  # 🔥 添加（注意：num_workers=0时不能用persistent_workers）
        )
    ...
    valid_loader = DataLoader(
        ...
        generator=generator,
        prefetch_factor=prefetch_factor,  # 🔥 添加
        persistent_workers=persistent_workers if num_workers > 0 else False,  # 🔥 添加
    )
    ...
    '''
    return train_data, valid_data, train_loader, valid_loader, n_classes
